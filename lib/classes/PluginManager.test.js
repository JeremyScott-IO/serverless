...
const { installPlugin } = require('../../test/utils/plugins');
...
describe('PluginManager', () => {
  ...
  describe('#spawn()', () => {
    it('should throw an error when the given command is not available', () => {
      ...
      expect(
        pluginManager.spawn(commandsArray)).to.be.rejectedWith(Error);
    });
    ...
    describe('when invoking a container', () => {
      it('should fail', () => {
        ...
        return expect(pluginManager.spawn(commandsArray)).to.be.rejectedWith(/command ".*" not found/);
      });
      ...
      it('should spawn nested commands', () => {
        ...
        return pluginManager.spawn(commandsArray).then(() => {
          expect(pluginManager.plugins[0].callResult).equal('>mysubcmdEvent1>mysubcmdEvent2');
        });
      });
    });
  ...
  describe('Plugin / CLI integration', function() {
    ...
    let serverlessInstance;
    let serviceDir;
    const serverlessExec = require('../../test/serverlessBinary');
