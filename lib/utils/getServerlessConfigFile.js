...
const getConfigFilePath = async function (servicePath, options = {}) {
  if (options.config) {
    const customPath = path.join(servicePath, options.config);
    return await fileExists(customPath).then(exists => {
...
const getServerlessConfigFile = memoizee(serverless =>
  getServerlessConfigFilePath(serverless).then( async (configFilePath) => {
